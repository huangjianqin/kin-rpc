...
public class DefaultRpcRequestProcessor extends RpcRequestProcessor {
    ...
    public synchronized void register(Invoker<?> invoker) {
        ...
        ExecutorSelector executorSelector = null;
        if (Objects.nonNull(executorConfig)) {
            executorSelector = ExecutorManager.getOrCreateExecutor(gsv, executorConfig);
        }
        serviceMetadataMap.put(serviceId, new ServiceMetadata(config, invoker, executorSelector));
    }
    ...
    public void process(RequestContext requestContext, RpcRequestCommand request) {
        ...
        ExecutorSelector executorSelector = serviceMetadata.getExecutorSelector();
        if (Objects.nonNull(executorSelector)) {
            Executor executor = executorSelector.select(request);
